...
class TestNumericRecommendationBranches(unittest.TestCase):
    ...
    def test_skewed_distributions_ask_for_distribution_aware_encoding(self):
        ...
        The key used to be spelled `prefered_distribution`, with one "r", and
        `NumericalFeature` takes `preferred_distribution`. Pasting the
        recommendation into a feature therefore left it on automatic detection,
        which is the one thing the recommendation exists to override.
        """
        for dist in ("heavy_tailed", "log_normal", "periodic", "sparse", "multimodal"):
            ...
            self.assertEqual(
                recommendation["config"]["preferred_distribution"], dist, dist
            )
            self.assertNotIn("prefered_distribution", recommendation["config"], dist)

    def test_the_recommended_distribution_reaches_the_generated_snippet(self):
        """The advisor computed it, stored it, and left it out of the code."""
        advisor = ModelAdvisor({"numeric_stats": {}, "categorical_stats": {}})
        advisor.recommendations = {
            "skewed": {
                "feature_type": "NumericalFeature",
                "preprocessing": ["FLOAT_NORMALIZED", "DISTRIBUTION_AWARE"],
                "config": {"preferred_distribution": "log_normal"},
                "notes": ["log-normal detected"],
            },
        }
        advisor.global_config = {"use_distribution_aware": True}
        snippet = advisor.generate_code_snippet()

        self.assertIn('preferred_distribution="log_normal"', snippet)

        # And the snippet has to build the feature it describes.
        namespace = {}
        exec(  # noqa: S102 - the snippet is generated by the code under test
            compile(
                snippet.split("# Create the preprocessor")[0],
                "<snippet>",
                "exec",
            ),
            namespace,
        ...
        self.assertEqual(
            namespace["features"]["skewed"].preferred_distribution,
            "log_normal",
        )
