...
```python
__init__(self, global_embedding_dim: int = 8, global_mlp_hidden_units: int = 16, global_num_bins: int = 10, global_init_min: float | list[float] = -3.0, global_init_max: float | list[float] = 3.0, global_dropout_rate: float = 0.1, global_use_batch_norm: bool = True, global_pooling: str = 'average', **kwargs)
```

Initialize the GlobalNumericalEmbedding layer.

### Parameters- **global_embedding_dim**: Dimension of the final global embedding.
...
### Parameters- **input_shape**: Shape of the inputs, ``(batch, ...)``.
