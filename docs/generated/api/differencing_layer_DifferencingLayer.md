...
### Parameters- **order**: The order of differencing to apply (default=1)
...
```python
__init__(self, order=1, drop_na=True, fill_value=0.0, keep_original=False, **kwargs)
```
...
### Parameters- **inputs**: Input tensor of shape (time_steps,) or (time_steps, features).
...
### Parameters- **input_shape**: Shape of the input, ``(time_steps,)`` or
        ``(time_steps, features)``.
