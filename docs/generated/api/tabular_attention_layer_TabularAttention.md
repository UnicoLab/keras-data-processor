...
This layer implements a dual attention- **mechanism**: 1. Inter-feature attention: Captures dependencies between features for each sample
...
```python
__init__(self, num_heads: int, d_model: int, dropout_rate: float = 0.1, **kwargs)
```
...
### Parameters- **inputs (tf.Tensor)**: Input tensor of shape (batch_size, num_samples, num_features)
