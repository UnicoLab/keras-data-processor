...
```python
__init__(self, num_heads: int, d_model: int, embedding_dim: int, dropout_rate: float = 0.1, **kwargs) -> None
```
...
```python
call(self, numerical_features: tensorflow.python.framework.tensor.Tensor, categorical_features: tensorflow.python.framework.tensor.Tensor, training: bool = False) -> tuple[tensorflow.python.framework.tensor.Tensor, tensorflow.python.framework.tensor.Tensor]
```

Process numerical and categorical features through multi-resolution attention.

### Parameters- **numerical_features**: Tensor of shape (batch_size, num_numerical, numerical_dim)
- **categorical_features**: Tensor of shape (batch_size, num_categorical, categorical_dim)
...
### Returns- **dict**: Configuration dictionary containing the layer parameters
